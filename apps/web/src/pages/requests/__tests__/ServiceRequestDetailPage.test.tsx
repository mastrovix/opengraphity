...
const request = (over: Record<string, unknown> = {}) => ({
  ...
  requestedBy: null, assignee: null, workflowInstance: null, availableTransitions: [], slaStatus: null,
  team: { __typename: 'Team', id: 't-desk', name: 'SUP_Service Desk' }, ...over,
})
const user = (id: string, name: string, role: UserRole, teams = ['t-desk']) => ({
  __typename: 'User', id, name, permissions: [...FACTORY_ROLE_PERMISSIONS[role]], active: true, teams: teams.map((tid) => ({ __typename: 'Team', id: tid })),
})
const usersMock: GqlMock = {
  request: { query: GET_ASSIGNABLE_USERS },
  result: { data: { users: [
    user('u-op', 'Olga Operator', 'operator'), user('u-view', 'Vera Viewer', 'viewer'), user('u-portal', 'Paolo Portale', 'end_user'),
    // Can receive tickets, but is not a member of the request's team (D56).
    user('u-other', 'Otto Other', 'operator', ['t-net']),
  ] } },
  maxUsageCount: Number.POSITIVE_INFINITY,
}
const teamsMock = teamChoicesMock([{ id: 't-desk', name: 'SUP_Service Desk', type: 'support' }, { id: 't-net', name: 'SUP_Network', type: 'support' }])
...
describe('ServiceRequestDetailPage — assegnatario', () => {
  it('offre solo chi lavora i ticket e invia la scelta', async () => {
    ...
    expect(within(select).queryByRole('option', { name: 'Otto Other' })).toBeNull()
