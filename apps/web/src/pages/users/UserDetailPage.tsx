...
function TeamTypeBadge({ type }: { type: string | null }) {
  if (!type) return <span style={{ color: '#8892a4', fontSize: 12 }}>—</span>
...
export function UserDetailPage() {
  ...
  if (!user) {
    return <div style={{ padding: '32px 40px', color: '#8892a4', fontSize: 14 }}>Utente non trovato.</div>
  }
  ...
  return (
    <div style={{ padding: '32px 40px', maxWidth: 1200 }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ fontSize: 11, color: '#8892a4', marginBottom: 4, cursor: 'pointer' }} onClick={() => navigate('/users')}>
          ← Users
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: '#0f1629', margin: 0 }}>{user.name}</h1>
      ...
      <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1 }}>
          <CollapsibleCard title={`Team (${user.teams.length})`} defaultOpen>
            {user.teams.length === 0 ? (
              <EmptyState icon={<Users size={24} color="#8892a4" />} title="Nessun team assegnato" />
            ) : (
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                ...
                <tbody>
                  {user.teams.map((t) => (
                    ...
                    >
                      <td style={{ padding: '8px 8px', fontWeight: 500, color: '#0f1629' }}>{t.name}</td>
          ...
          <Card>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                ...
                <div style={{ fontSize: 11, fontFamily: 'monospace', color: '#4a5468', wordBreak: 'break-all' }}>{user.id}</div>
              </div>
              <div>
                <div style={{ fontSize: 11, fontWeight: 600, color: '#8892a4', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 4 }}>Email</div>
                <div style={{ fontSize: 13, color: '#374151' }}>{user.email}</div>
              </div>
              <div>
                <div style={{ fontSize: 11, fontWeight: 600, color: '#8892a4', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 4 }}>Ruolo</div>
                <RoleBadge role={user.role} />
              </div>
              <div>
