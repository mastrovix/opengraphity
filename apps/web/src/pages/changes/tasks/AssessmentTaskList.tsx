    ...
    <div style={{ ...cardStyle, borderLeft: canEditAssessment ? '4px solid var(--color-brand)' : '4px solid #e5e7eb', borderRadius: '0 10px 10px 0', background: canEditAssessment ? '#fff' : '#fafafa', padding: 0, transition: 'all 0.2s' }}>
      <div onClick={() => onSetAssessmentOpen(!assessmentOpen)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', padding: '14px 20px', borderBottom: assessmentOpen ? '1px solid #e5e7eb' : 'none' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 'var(--font-size-card-title)', fontWeight: 600, color: 'var(--color-slate-dark)' }}>Assessment Tasks</span>
            ...
            <div style={{ position: 'fixed', right: 0, top: 0, bottom: 0, width: 560, backgroundColor: '#fff', zIndex: 1001, boxShadow: '-4px 0 24px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column' }}>
              <div style={{ padding: '20px 24px', borderBottom: '1px solid #e2e6f0', flexShrink: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <div style={{ fontSize: 'var(--font-size-body)', fontWeight: 700, color: 'var(--color-slate-light)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>Assessment Task</div>
                    <h2 style={{ fontSize: 'var(--font-size-card-title)', fontWeight: 600, color: 'var(--color-slate-dark)', margin: '0 0 6px' }}>{task.ci?.name ?? '—'}</h2>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      {task.ci?.type && <span style={{ fontSize: 'var(--font-size-body)', color: 'var(--color-slate-light)', backgroundColor: '#f3f4f6', padding: '2px 8px', borderRadius: 4 }}>{task.ci.type}</span>}
                      {task.ci?.environment && <span style={{ fontSize: 'var(--font-size-body)', color: 'var(--color-slate-light)' }}>{task.ci.environment}</span>}
                      <Badge value={task.status} map={TASK_STATUS_COLORS} />
                    </div>
                  </div>
                  <button onClick={() => setAssessmentTaskPopup(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 'var(--font-size-page-title)', color: 'var(--color-slate-light)', lineHeight: 1, padding: 4 }}>×</button>
                </div>
              </div>
              <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ fontSize: 'var(--font-size-body)', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", color: 'var(--color-slate-light)' }}>{task.id}</div>
                </div>
                <div style={{ fontSize: 'var(--font-size-body)', fontWeight: 700, color: 'var(--color-slate-light)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 14 }}>Assessment</div>
                <div style={{ marginBottom: 14 }}>
                  <div style={{ fontSize: 'var(--font-size-body)', fontWeight: 700, color: 'var(--color-slate-light)', textTransform: 'uppercase', marginBottom: 6 }}>Team</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    {task.assignedTeam ? (
                      <span style={{ fontSize: 'var(--font-size-card-title)', fontWeight: 600, color: 'var(--color-slate-dark)' }}>{task.assignedTeam.name}</span>
                    ) : (
                    ...
                    {isEditable && (
                      <button
                        ...
                        style={{ fontSize: 'var(--font-size-body)', padding: '2px 8px', borderRadius: 4, border: '1px solid #e2e6f0', background: 'transparent', cursor: 'pointer', color: 'var(--color-slate-light)' }}
                      >
                        Riassegna
                      </button>
                    )}
                ...
                {isEditable && task.assignedTeam && (
                  <div style={{ marginBottom: 14 }}>
                    <div style={{ fontSize: 'var(--font-size-body)', fontWeight: 700, color: 'var(--color-slate-light)', textTransform: 'uppercase', marginBottom: 6 }}>Assegnato a</div>
                    {task.assignee ? (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <span style={{ fontSize: 'var(--font-size-body)', color: 'var(--color-slate-dark)' }}>{task.assignee.name}</span>
                        <button onClick={() => setAssignTaskUserId((prev) => ({ ...prev, [task.id]: '' }))} style={{ fontSize: 'var(--font-size-body)', padding: '2px 8px', borderRadius: 4, border: '1px solid #e2e6f0', background: 'transparent', cursor: 'pointer', color: 'var(--color-slate-light)' }}>Cambia</button>
                      </div>
                    ) : (
                      <select
                ...
                <div style={{ marginBottom: 14 }}>
                  <div style={{ fontSize: 'var(--font-size-body)', fontWeight: 700, color: 'var(--color-slate-light)', textTransform: 'uppercase', marginBottom: 6 }}>Risk Level</div>
                  {isEditable ? (
                    ...
                    <span style={{ fontSize: 'var(--font-size-body)', color: 'var(--color-slate-dark)' }}>{task.riskLevel ?? '—'}</span>
                  )}
                </div>
                <div style={{ marginBottom: 14 }}>
                  <div style={{ fontSize: 'var(--font-size-body)', fontWeight: 700, color: 'var(--color-slate-light)', textTransform: 'uppercase', marginBottom: 6 }}>Descrizione impatto</div>
                  {isEditable ? (
                    ...
                    <p style={{ fontSize: 'var(--font-size-body)', color: 'var(--color-slate-dark)', margin: 0, lineHeight: 1.5 }}>{task.impactDescription ?? '—'}</p>
                  )}
                </div>
                <div style={{ marginBottom: 14 }}>
                  <div style={{ fontSize: 'var(--font-size-body)', fontWeight: 700, color: 'var(--color-slate-light)', textTransform: 'uppercase', marginBottom: 6 }}>Mitigazione</div>
                  {isEditable ? (
                    ...
                    <p style={{ fontSize: 'var(--font-size-body)', color: 'var(--color-slate-dark)', margin: 0, lineHeight: 1.5 }}>{task.mitigation ?? '—'}</p>
                  )}
                </div>
                <div style={{ marginBottom: 20 }}>
                  <div style={{ fontSize: 'var(--font-size-body)', fontWeight: 700, color: 'var(--color-slate-light)', textTransform: 'uppercase', marginBottom: 6 }}>Note</div>
                  {isEditable ? (
                    ...
                    <p style={{ fontSize: 'var(--font-size-body)', color: 'var(--color-slate-dark)', margin: 0, lineHeight: 1.5 }}>{task.notes ?? '—'}</p>
                  )}
                </div>
                {isEditable && (
                  <div style={{ marginBottom: 24 }}>
                    {!task.assignedTeam && (
                      <div style={{ fontSize: 'var(--font-size-body)', color: 'var(--color-trigger-sla-breach)', marginBottom: 8 }}>⚠ Assegna un team prima di completare</div>
                    )}
                    {task.assignedTeam && (
                      <>
                        {deploySteps.length === 0 && (
                          <div style={{ fontSize: 'var(--font-size-body)', color: '#ca8a04', padding: '8px 12px', borderRadius: 6, backgroundColor: 'rgba(234,179,8,0.08)', marginBottom: 8 }}>
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                          ...
                          <button
                            ...
                            style={{ padding: '9px 16px', borderRadius: 7, border: 'none', fontSize: 'var(--font-size-card-title)', fontWeight: 600, cursor: 'pointer', backgroundColor: '#fef2f2', color: 'var(--color-trigger-sla-breach)' }}
                          >
                            Rigetta
                          </button>
                        </div>
                ...
                <div style={{ borderTop: '1px solid #e2e6f0', paddingTop: 20, marginTop: 4 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                    ...
                    <span style={{ fontSize: 'var(--font-size-body)', fontWeight: 700, color: 'var(--color-slate-light)', textTransform: 'uppercase', letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>Piano di Deployment</span>
                    <div style={{ flex: 1, height: 1, backgroundColor: '#e2e6f0' }} />
                  </div>
                  {deploySteps.length > 0 ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 14 }}>
                      {deploySteps.map((step) => (
                        <div key={step.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', backgroundColor: '#f8f9fc', borderRadius: 7 }}>
                          <span style={{ backgroundColor: 'var(--color-brand)', color: '#fff', width: 22, height: 22, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'var(--font-size-body)', fontWeight: 700, flexShrink: 0 }}>{step.order}</span>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontSize: 'var(--font-size-card-title)', fontWeight: 500, color: 'var(--color-slate-dark)' }}>{step.title}</div>
                            <div style={{ fontSize: 'var(--font-size-body)', color: 'var(--color-slate-light)' }}>
                    ...
                    <p style={{ fontSize: 'var(--font-size-body)', color: 'var(--color-slate-light)', marginBottom: 14 }}>Nessuno step pianificato.</p>
                  )}
                  {deployStepsEditable && (
                    <div style={{ marginTop: 8, padding: 16, borderRadius: 8, border: '1px dashed #e2e6f0', backgroundColor: '#fafafa' }}>
                      <div style={{ fontSize: 'var(--font-size-body)', fontWeight: 600, color: 'var(--color-slate-light)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 12 }}>Aggiungi step</div>
                      <input
                        placeholder="Titolo step *"
                        value={newStepForm.title}
                        onChange={(e) => setNewStepForm((p) => ({ ...p, title: e.target.value }))}
                        style={{ width: '100%', boxSizing: 'border-box', padding: '8px 12px', borderRadius: 8, border: '1px solid #e2e6f0', fontSize: 'var(--font-size-card-title)', marginBottom: 8, fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", outline: 'none' }}
                      />
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 8 }}>
                        <div>
                          <label style={{ fontSize: 'var(--font-size-body)', color: 'var(--color-slate-light)', display: 'block', marginBottom: 4 }}>DATA INIZIO *</label>
                          <input type="date" value={newStepForm.scheduledStart}
                            ...
                            style={{ width: '100%', boxSizing: 'border-box', padding: '8px', borderRadius: 8, border: '1px solid #e2e6f0', fontSize: 'var(--font-size-card-title)', outline: 'none' }} />
                        </div>
                        <div>
                          <label style={{ fontSize: 'var(--font-size-body)', color: 'var(--color-slate-light)', display: 'block', marginBottom: 4 }}>DURATA (GG) *</label>
                        ...
                        <div>
                          <label style={{ fontSize: 'var(--font-size-body)', color: 'var(--color-slate-light)', display: 'block', marginBottom: 4 }}>FINE PREVISTA</label>
                          <input readOnly value={newStepForm.scheduledEnd}
                            style={{ width: '100%', boxSizing: 'border-box', padding: '8px', borderRadius: 8, border: '1px solid #e2e6f0', fontSize: 'var(--font-size-card-title)', backgroundColor: '#f3f4f6', color: 'var(--color-slate-light)' }} />
                        </div>
                      </div>
                      <div style={{ marginBottom: 8 }}>
                        <label style={{ fontSize: 'var(--font-size-body)', color: 'var(--color-slate-light)', display: 'block', marginBottom: 4 }}>TEAM DEPLOY</label>
                        <select value={newStepForm.assignedTeamId} onChange={(e) => setNewStepForm((p) => ({ ...p, assignedTeamId: e.target.value }))}
                          style={{ width: '100%', padding: '8px 12px', borderRadius: 8, border: '1px solid #e2e6f0', fontSize: 'var(--font-size-card-title)', outline: 'none' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: newStepForm.hasValidation ? 8 : 0 }}>
                        ...
                        <label htmlFor="popupHasValidation" style={{ fontSize: 'var(--font-size-card-title)', cursor: 'pointer' }}>Ha finestra di validazione propria</label>
                      </div>
                      {newStepForm.hasValidation && (
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8, marginTop: 8 }}>
                          <div>
                            <label style={{ fontSize: 'var(--font-size-body)', color: 'var(--color-slate-light)', display: 'block', marginBottom: 4 }}>INIZIO VALIDAZIONE *</label>
                            <input type="date" value={newStepForm.validationStart} onChange={(e) => setNewStepForm((p) => ({ ...p, validationStart: e.target.value }))} style={{ width: '100%', boxSizing: 'border-box', padding: '8px', borderRadius: 8, border: '1px solid #e2e6f0', fontSize: 'var(--font-size-card-title)', outline: 'none' }} />
                          </div>
                          <div>
                            <label style={{ fontSize: 'var(--font-size-body)', color: 'var(--color-slate-light)', display: 'block', marginBottom: 4 }}>FINE VALIDAZIONE *</label>
                            <input type="date" value={newStepForm.validationEnd} onChange={(e) => setNewStepForm((p) => ({ ...p, validationEnd: e.target.value }))} style={{ width: '100%', boxSizing: 'border-box', padding: '8px', borderRadius: 8, border: '1px solid #e2e6f0', fontSize: 'var(--font-size-card-title)', outline: 'none' }} />
                          </div>
                          <div style={{ gridColumn: '1/-1' }}>
                            <label style={{ fontSize: 'var(--font-size-body)', color: 'var(--color-slate-light)', display: 'block', marginBottom: 4 }}>TEAM VALIDAZIONE</label>
                  ...
                  {deployStepsEditable && deploySteps.length > 0 && !deploySteps.some((s) => s.hasValidation) && (
                    <div style={{ marginTop: 16, padding: 16, borderRadius: 8, border: '1px solid #fde68a', backgroundColor: 'rgba(234,179,8,0.05)' }}>
                      <div style={{ fontSize: 'var(--font-size-body)', fontWeight: 600, color: '#ca8a04', marginBottom: 12 }}>
                        ⚠ Nessuno step ha validazione propria — definisci la finestra di validazione globale
                      </div>
                      {validationTask ? (
                        <div style={{ fontSize: 'var(--font-size-body)', color: 'var(--color-slate)' }}>
                        ...
                        <>
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
                            <div>
                              <label style={{ fontSize: 'var(--font-size-body)', color: 'var(--color-slate-light)', display: 'block', marginBottom: 4 }}>INIZIO VALIDAZIONE *</label>
                              <input type="date" value={globalValidationStart} onChange={(e) => setGlobalValidationStart(e.target.value)} style={{ width: '100%', boxSizing: 'border-box', padding: '8px', borderRadius: 8, border: '1px solid #e2e6f0', fontSize: 'var(--font-size-card-title)', outline: 'none' }} />
                            </div>
                            <div>
                              <label style={{ fontSize: 'var(--font-size-body)', color: 'var(--color-slate-light)', display: 'block', marginBottom: 4 }}>FINE VALIDAZIONE *</label>
                              <input type="date" value={globalValidationEnd} onChange={(e) => setGlobalValidationEnd(e.target.value)} style={{ width: '100%', boxSizing: 'border-box', padding: '8px', borderRadius: 8, border: '1px solid #e2e6f0', fontSize: 'var(--font-size-card-title)', outline: 'none' }} />
                            </div>
                          </div>
                          <button
