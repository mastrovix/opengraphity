...
export function ChangeCIList({ changeId: _changeId, affectedCIs, currentStep, onAddCI, onRemoveCI }: Props) {
          ...
          {showSearch && (
            <div style={{ marginBottom: 12 }}>
              <input
                ...
                style={inputStyle}
              />
              {ciResults.filter((ci) => !affectedCIs.find((a) => a.id === ci.id)).length > 0 && (
                <div style={{ border: '1px solid #e2e6f0', borderRadius: 6, marginTop: 4, backgroundColor: '#fff', maxHeight: 160, overflowY: 'auto' }}>
                  {ciResults
                    ...
                    .map((ci) => (
                      <div
                        key={ci.id}
                        onClick={() => { onAddCI(ci.id); setCiSearch(''); setShowSearch(false) }}
