...
export function ServersPage() {
  ...
  const { data, loading } = useQuery<{ servers: { items: ServerItem[]; total: number } }>(GET_SERVERS, {
    variables: { limit: PAGE_SIZE, offset: page * PAGE_SIZE, ...queryFilters },
    fetchPolicy: 'cache-and-network',
  })

  const items = data?.servers?.items ?? []
  const total = data?.servers?.total ?? 0
  const totalPages = Math.max(1, Math.ceil(total / PAGE_SIZE))

  const COLUMNS: ColumnDef<ServerItem>[] = [
    ...
    { key: 'location', label: 'Location', sortable: true, render: (v) => v as string || <span style={{ color: '#c4cad4' }}>—</span> },
    { key: 'vendor', label: 'Vendor', sortable: true, render: (v) => v as string || <span style={{ color: '#c4cad4' }}>—</span> },
    { key: 'osVersion', label: 'OS', sortable: false, render: (v) => v as string || <span style={{ color: '#c4cad4' }}>—</span> },
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 24 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: '#0f1629', margin: 0 }}>Server</h1>
        {total > 0 && <span style={{ fontSize: 13, color: '#8892a4' }}>{total} totali</span>}
      </div>
      {!loading && items.length === 0 ? (
        <EmptyState icon={<ServerIcon size={32} color="#8892a4" />} title="Nessun server" />
      ) : (
