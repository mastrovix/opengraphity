...
const EMPTY_FILTER: HealthFilter = { health: null, type: '', environment: '', team: '', search: '' }
...
interface TileProps {
  ...
  value:        number
  context:      string
  /** Tooltip della riga di contesto (dichiara come è calcolata). */
  contextHint?: string
...
function HealthTile({ tileKey, label, value, context, contextHint, hint, active, onClick }: TileProps) {
  ...
  const body = (
    <>
      ...
      <div title={contextHint} style={{ fontSize: 'var(--font-size-table)', color: 'var(--color-slate-light)', marginTop: 10 }}>{context}</div>
    </>
  )
...
function ImpactChip({ dependents }: { dependents: number }) {
  const { t } = useTranslation()
  const high = dependents >= HIGH_IMPACT
  return (
    <span
      title={high ? t('monitoring.health.dependentsHigh') : t('monitoring.health.dependentsHint', { count: dependents })}
      style={{
...
function SourceCell({ source }: { source: CIHealthRow['healthSource'] }) {
  const { t } = useTranslation()
  ...
  return (
    <span
      title={manual ? t('monitoring.health.sourceManualHint') : t('monitoring.health.sourceMonitoringHint')}
  ...
  return (
    ...
    >
      ...
      <td style={TD}>
        ...
        {since && (
          <div title={t('monitoring.health.sinceHint', { date: formatDateTime(row.healthSince) })} style={{ fontSize: 'var(--font-size-table)', color: 'var(--color-slate-light)', marginTop: 3 }}>
      ...
      <td style={TD}><ImpactChip dependents={row.dependents} /></td>
      ...
      <td style={TD}><SourceCell source={row.healthSource} /></td>
    </tr>
  )
}
...
export function CIHealthPage() {
  ...
  const [filter, setFilter] = useState<HealthFilter>(EMPTY_FILTER)
  ...
  // Somma dei dipendenti delle righe di un certo stato nella pagina corrente (dichiarato nella tooltip).
  const dependentsOf = (h: CIHealth) => items.filter((r) => r.health === h).reduce((sum, r) => sum + r.dependents, 0)

  const tileContext = (key: TileKey): { context: string; hint?: string } => {
    if (key === 'unmonitored') return { context: t('monitoring.health.tiles.unmonitoredContext') }
    if (key === 'operational') return { context: t('monitoring.health.tiles.operationalContext', { count: monitored }) }
    const n = overview?.[key] ?? 0
    if (n === 0) return { context: t('monitoring.health.tiles.noneContext') }
    return { context: t('monitoring.health.tiles.dependentsContext', { count: dependentsOf(key) }), hint: t('monitoring.health.tiles.dependentsHint') }
  }
  ...
  return (
    <PageContainer>
      <ListPageHeader
        ...
        actions={
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            ...
            <Button variant="secondary" size="xs" icon={<Share2 size={13} aria-hidden="true" />} onClick={() => navigate('/topology?health=1')}>{t('monitoring.health.viewOnMap')}</Button>
          </div>
        }
      />

      {/* Riquadri: i tre stati filtrano la tabella, "senza monitoraggio" è informativo. */}
      {overview && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: 12, marginBottom: 20 }}>
          {TILE_ORDER.map((key) => {
            const { context, hint } = tileContext(key)
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 10, marginBottom: 12 }}>
        <Select aria-label={t('monitoring.health.filters.type')} value={filter.type} onChange={(e) => updateFilter({ type: e.target.value })} style={{ width: 180 }}>
        ...
        <Select aria-label={t('monitoring.health.filters.environment')} value={filter.environment} onChange={(e) => updateFilter({ environment: e.target.value })} style={{ width: 170 }} title={baseEnums.error ?? undefined}>
        ...
        <Select aria-label={t('monitoring.health.filters.team')} value={filter.team} onChange={(e) => updateFilter({ team: e.target.value })} style={{ width: 180 }}>
