...
/**
 * THE CI IN FOUR TABS (26 Sep 2026, review of the pages): what the CI is (with
 * its health and the services that use it), how it is tied to the rest, the
 * tickets on it, its attachments — nine cards were one column.
 */
const CI_TABS = ['information', 'relations', 'tickets', 'attachments'] as const
...
export function CIDetailPage() {
  ...
  const [tab, setTab] = useTabParam(CI_TABS, 'information')
  const tabsId = useId()
  ...
  return (
    <PageContainer>
      ...
      {/* Four tabs; the open one is in the address (?tab=) */}
      <Tabs
        idPrefix={tabsId}
        ariaLabel={t('detail.tabs.ciLabel')}
        value={tab}
        onChange={setTab}
        items={[
          { key: 'information', label: t('detail.tabs.information') },
          { key: 'relations', label: t('detail.tabs.relations'), badge: (ci.dependencies as CIRelation[]).length + (ci.dependents as CIRelation[]).length },
          { key: 'tickets', label: t('detail.tabs.tickets') },
