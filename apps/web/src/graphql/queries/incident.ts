...
export const GET_SERVICE_REQUEST = gql`
  query GetServiceRequest($id: ID!) {
    serviceRequest(id: $id) {
      ...
      formRevision
      formAnswers { name label fieldType value values displayValue displayValues references { id label } files { id filename sizeBytes } tableColumns { name label fieldType } rows { cells { column value displayValue } } }
    }
  }
  ${CUSTOM_FIELD_VALUE_FIELDS}
`
...
export const GET_ENTITY_FILTER_FIELDS = gql`
  query EntityFilterFields($typeName: String!) {
    entityFilterFields(typeName: $typeName) { name kind scalarName enumValues label choices { value label } formFieldType vocabulary rowFilter multi }
  }
`
