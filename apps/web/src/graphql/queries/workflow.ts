...
export const GET_WORKFLOW_DEFINITION_BY_ID = gql`
  query GetWorkflowDefinitionById($id: ID!) {
    workflowDefinitionById(id: $id) {
      ...
      steps { id name label labels { language label } type enterActions exitActions isInitial isTerminal isOpen category purpose deadline timerDelayMinutes order currentInstances positionX positionY }
      transitions {
