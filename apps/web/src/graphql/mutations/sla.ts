...
export const CREATE_OLA_CONTRACT = gql`
  mutation CreateOLAContract($input: CreateOLAContractInput!) {
    createOLAContract(input: $input) {
      id type name entityType responseMinutes resolveMinutes businessHours calendarId calendarName timezone complianceTarget complianceWarning partyType partyName teamId teamName enabled createdAt
    }
  }
`
