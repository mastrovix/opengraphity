...
export function useAIDisabledText(feature: AIFeature): string {
  ...
  const { can } = useMe()
  return t(can('config.organization') ? 'components.aiDisabled.adminShort' : 'components.aiDisabled.userShort', { feature: t(`pages.organization.aiFeature.${feature}`) })
}

export function AIDisabledNotice({ feature }: { feature: AIFeature }) {
  const { t } = useTranslation()
  const { can } = useMe()
  const mayTurnOn = can('config.organization')
