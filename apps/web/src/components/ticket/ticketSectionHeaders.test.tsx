...
describe('testate dei riquadri dei ticket', () => {
  it('cronologia del workflow: testata condivisa, stato aperto controllato dal chiamante', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /Workflow history/ }))
    expect(toggled).toBe(1)
    expect(screen.getByRole('button', { name: /Workflow history/ })).toHaveAttribute('aria-expanded', 'false')

    rerender(<WorkflowTimeline historyDesc={[EXEC]} entityType="incident" timelineOpen onToggle={() => {}} />)
    expect(screen.getByRole('button', { name: /Workflow history/ })).toHaveAttribute('aria-expanded', 'true')
  ...
  it('CI impattati: testata condivisa col conteggio, e «Aggiungi CI» fuori dal pulsante che apre', async () => {
    ...
    const toggle = screen.getByRole('button', { name: /Impacted CIs/ })
    expect(toggle).toHaveAttribute('aria-expanded', 'false')
    ...
    const add = screen.getByRole('button', { name: '+ Add CI' })
    expect(toggle.contains(add)).toBe(false)
    await user.click(add)
    expect(screen.getByRole('button', { name: /Impacted CIs/ })).toHaveAttribute('aria-expanded', 'true')
    expect(screen.getByPlaceholderText(/Search CIs/)).toBeInTheDocument()
  })
})
