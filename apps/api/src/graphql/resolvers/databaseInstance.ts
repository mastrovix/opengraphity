import { withSession, mapDatabaseInstance, CI_FIELD_RESOLVERS, runQuery, runQueryOne } from './ci-utils.js'
...
async function databaseInstances(_: unknown, args: { limit?: number; offset?: number; environment?: string; status?: string; search?: string }, ctx: GraphQLContext) {
  ...
  return withSession(async (session) => {
    ...
    return { items: items.map((r) => mapDatabaseInstance(r.props)), total }
  })
}
...
export const databaseInstanceResolvers = {
  Query: { databaseInstances, databaseInstance },
  DatabaseInstance: CI_FIELD_RESOLVERS,
}
