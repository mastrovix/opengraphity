 ...
 * grafo, tutti post-commit e tutti a prova di guasto: timer OLA annullati,
 * allarmi silenziati rivalutati, problem che tornano in analisi e — dalla
 * revisione 2 (D6.1) — i Servizi monitorati avvisati che la finestra di questa
 * change non c'è più (i suoi componenti tornano a pesare).
 */
...
describe('deleteChange', () => {
  it('avvisa i Servizi monitorati che la finestra è sparita, DOPO il commit e senza mai lanciare (D6.1)', async () => {
    ...
    expect(reevaluateSuppressedEvents).toHaveBeenCalledWith('t1', 'chg-1', 'adm-1')
  })
