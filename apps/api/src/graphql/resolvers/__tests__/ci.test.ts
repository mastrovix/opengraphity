...
describe('ciIncidents', () => {
  it('follows the incident relationship, tenant-scoped, and maps with the list mapper', async () => {
    ...
    expect(squash(cypher)).toContain(`MATCH (n:ConfigurationItem {id: $ciId, tenant_id: $tenantId})<-[:${TICKET_CI_RELATIONSHIP.incident}]-(i:Incident {tenant_id: $tenantId})`)
...
describe('ciChanges', () => {
  it('follows the change relationship, excludes deleted changes and uses the change mapper', async () => {
    ...
    expect(squash(cypher)).toContain(`MATCH (n:ConfigurationItem {id: $ciId, tenant_id: $tenantId})<-[:${TICKET_CI_RELATIONSHIP.change}]-(c:Change {tenant_id: $tenantId})`)
...
describe('ciProblems', () => {
  it('scopes both the problem and the CI by tenant, via the problem relationship', async () => {
    ...
    const [, cypher, params] = lastCall()
    expect(squash(cypher)).toContain(`MATCH (n:ConfigurationItem {id: $ciId, tenant_id: $tenantId})<-[:${TICKET_CI_RELATIONSHIP.problem}]-(p:Problem {tenant_id: $tenantId})`)
    expect(params).toEqual({ ciId: 'ci-1', tenantId: 'tenant-a' })
  })
...
describe('ciServiceRequests', () => {
  it('follows the request relationship (CM-8), tenant-scoped on both ends, with the request mapper', async () => {
    ...
    const [, cypher, params] = lastCall()
    expect(squash(cypher)).toContain(`MATCH (n:ConfigurationItem {id: $ciId, tenant_id: $tenantId})<-[:${TICKET_CI_RELATIONSHIP.service_request}]-(r:ServiceRequest {tenant_id: $tenantId})`)
