import { withSession, mapDatabase, CI_FIELD_RESOLVERS, runQuery, runQueryOne } from './ci-utils.js'
...
async function databases(_: unknown, args: { limit?: number; offset?: number; environment?: string; status?: string; search?: string }, ctx: GraphQLContext) {
  ...
  return withSession(async (session) => {
    ...
    return { items: items.map((r) => mapDatabase(r.props)), total }
  })
}

async function database(_: unknown, args: { id: string }, ctx: GraphQLContext) {
  return withSession(async (session) => {
    const row = await runQueryOne<{ props: Props }>(session,
      `MATCH (n:Database {id: $id, tenant_id: $tenantId}) RETURN properties(n) AS props`,
      { id: args.id, tenantId: ctx.tenantId }
    )
    return row ? mapDatabase(row.props) : null
  })
}

export const databaseResolvers = {
  Query: { databases, database },
  Database: CI_FIELD_RESOLVERS,
}
