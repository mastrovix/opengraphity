export function discoverySDL(): string {
  ...
  type SyncConflict {
    ...
    ciType:         String!
    """
    locked_fields = un campo bloccato è cambiato alla sorgente; unknown_ci_type =
    il ci_type in arrivo non è un tipo di questo cliente, il CI NON è stato
    creato (ondata 6 · A-11); invalid_properties = le proprietà scoperte non
    sono scrivibili nel grafo (un valore che non è primitivo, una chiave fuori
    forma): il CI non è stato scritto e il resto del run è continuato
    (revisione totale · D-20).
    """
    kind:           String!
