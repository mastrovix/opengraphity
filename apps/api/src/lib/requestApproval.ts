...
/** True se la transizione verso `toStep` salterebbe l'approvazione richiesta. */
export async function requestApprovalWouldBeSkipped(
  session: Session, tenantId: string, instanceId: string, toStep: string,
): Promise<boolean> {
  const row = await runQueryOne<{ requires: unknown; approvedPassages: unknown; targetPurpose: string | null; targetTerminal: unknown }>(session, `
