 ...
 *  - DOVE SI VEDE: sempre; solo in certe fasi; oppure da una fase in poi (le
 *    fasi con `step_order` — l'ordine del disegnatore — uguale o maggiore,
 *    nella definizione del ticket; senza ordine vale 999, come nello schema);
...
/** Le fasi della definizione del ticket, in ordine, e quella in cui si trova (o in cui nasce). */
export interface StepContext { steps: { name: string; order: number }[]; current: string }
...
export function fieldStepState(visibility: StepVisibility, editability: StepEditability, ctx: StepContext | null): { visible: boolean; editable: boolean } {
  ...
  switch (visibility.mode) {
    ...
    case 'from': {
      const from = ctx.steps.find((s) => s.name === visibility.step)
...
/** Le fasi (con l'ordine) della definizione del ticket e la fase corrente; null senza istanza. */
export async function ticketStepContext(session: Session, tenantId: string, ticketId: string): Promise<StepContext | null> {
  const res = await session.executeRead((tx) => tx.run(`
    ...
    MATCH (wd:WorkflowDefinition {id: wi.definition_id, tenant_id: $tenantId})-[:HAS_STEP]->(s:WorkflowStep)
    RETURN wi.current_step AS current, collect({name: s.name, order: coalesce(s.step_order, 999)}) AS steps
  `, { ticketId, tenantId }))
  const r = res.records[0]
  if (!r || r.get('current') == null) return null
  return { current: String(r.get('current')), steps: (r.get('steps') as Array<{ name: string; order: unknown }>).map((s) => ({ name: s.name, order: Number(s.order) })) }
}
...
export async function creationStepContext(session: Session, tenantId: string, entityType: string, category: string | null): Promise<StepContext | null> {
  const res = await session.executeRead((tx) => tx.run(`
    ...
    WITH wd, collect({name: s.name, order: coalesce(s.step_order, 999), initial: coalesce(s.is_initial, s.type = 'start')}) AS steps
    RETURN steps
  `, { tenantId, entityType, category }))
  const steps = (res.records[0]?.get('steps') ?? []) as Array<{ name: string; order: unknown; initial: boolean }>
  const initial = steps.find((s) => s.initial)
  if (!initial) return null
  return { current: initial.name, steps: steps.map((s) => ({ name: s.name, order: Number(s.order) })) }
}
