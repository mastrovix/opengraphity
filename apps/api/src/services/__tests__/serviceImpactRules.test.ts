...
import { evaluateImpact, impactPath, nodeContributes, type ImpactNodeInput } from '../serviceImpact/rules.js'
import { DEFAULT_SERVICE_IMPACT_RULES, SERVICE_MAX_CAUSES, type ServiceImpactRules } from '../../lib/serviceVocabularies.js'
...
function node(over: Partial<ImpactNodeInput> & { ciId: string }): ImpactNodeInput {
  return { level: 2, role: 'infrastructure', propagate: 'weighted', weight: 5, critical: false, health: 'operational', inMaintenance: false, via: 'api-03', ...over }
}
...
describe('evaluateImpact — tabella del contratto', () => {
  ...
  it('Billing tutto operativo → operational 0, nessuna causa; il certificato (never) non pesa nemmeno se giù', () => {
    expect(evaluateImpact(billing(), RULES)).toEqual({ health: 'operational', impactScore: 0, causes: [] })
    expect(evaluateImpact(billing({ cert: 'down' }), RULES)).toEqual({ health: 'operational', impactScore: 0, causes: [] })
  })
  ...
  it('nessun nodo con salute nota → unknown 0, con ENTRAMBE le modalità (un servizio non monitorato non è «operativo»)', () => {
    ...
    expect(evaluateImpact(nodes, { ...RULES, unknown_nodes: 'ignore' })).toEqual({ health: 'unknown', impactScore: 0, causes: [] })
    expect(evaluateImpact(nodes, { ...RULES, unknown_nodes: 'operational' })).toEqual({ health: 'unknown', impactScore: 0, causes: [] })
  ...
  it('tutti i nodi con propagate = never → unknown', () => {
    ...
    expect(evaluateImpact(nodes, RULES)).toEqual({ health: 'unknown', impactScore: 0, causes: [] })
  })

  it('nodo critico in finestra di change → maintenance, qualunque sia il resto; il nodo in finestra non pesa', () => {
    const r = evaluateImpact(billing({ db: 'down' }, { api: { inMaintenance: true } }), RULES)
    expect(r.health).toBe('maintenance')
    expect(r.impactScore).toBe(Math.round((100 * 5) / 8))   // api-03 (8) fuori dal totale
    expect(r.causes.map((c) => c.ciId)).toEqual(['db-01'])
  })

  it('nodo NON critico in finestra → non pesa e non è causa, il servizio segue il resto', () => {
    const r = evaluateImpact(billing({ db: 'down' }, { db: { inMaintenance: true } }), RULES)
    expect(r).toEqual({ health: 'operational', impactScore: 0, causes: [] })
  })
...
describe('impactPath e nodeContributes', () => {
  ...
  it('nodeContributes: never / in finestra / senza salute (ignore) → false; senza salute con operational → true', () => {
    expect(nodeContributes(node({ ciId: 'a' }), RULES)).toBe(true)
    expect(nodeContributes(node({ ciId: 'a', propagate: 'never' }), RULES)).toBe(false)
    expect(nodeContributes(node({ ciId: 'a', inMaintenance: true }), RULES)).toBe(false)
