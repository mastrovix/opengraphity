...
async function seed(TENANT_ID: string) {
  ...
  if (serverIds.length === 0) throw new Error(`No server CIs found for tenant ${TENANT_ID}`)
  ...
  for (let i = 1; i <= 400; i++) {
    ...
    for (const serverId of depServerIds) {
      await session.run(
        `MATCH (c:Database {id: $ciId}), (s:Server {id: $serverId})
...
runScript('seed-databases', async () => {
  refuseInProduction('seed-databases')
