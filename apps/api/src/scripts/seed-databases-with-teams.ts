...
async function seed(TENANT_ID: string) {
  ...
  for (let i = 1; i <= 500; i++) {
    const name        = `DTB-${String(i).padStart(3, '0')}`
    const id          = uuidv4()
    ...
    const result = await session.run(
      ...
      { name, tenantId: TENANT_ID, id, environment, status, description: `Database ${name}`, createdAt: now }
    )
    ...
    await session.run(
      `MATCH (c:Database {id: $ciId}), (t:Team {id: $teamId})
       MERGE (c)-[:OWNED_BY]->(t)`,
    ...
    if (i % 100 === 0) console.log(`  ${i}/500 processed...`)
  }
...
runScript('seed-databases-with-teams', async () => {
  refuseInProduction('seed-databases-with-teams')
