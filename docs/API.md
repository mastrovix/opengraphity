...
|----------|-------------|
| `createIncident(input)` | Open a new incident. It goes to `input.teamId` when given (the form prefills it with the support group of the selected CI), otherwise to the support group of the first impacted CI that has one — the same for every channel: REST, Slack, inbound webhooks, monitoring alarms, monitored services. The incident stays in its first step (the group's queue): the SLA response is a person taking it, not this routing (`incident.assigned` carries `routed_at_creation: true`) |
